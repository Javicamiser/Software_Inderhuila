...
const PAGE_TITLES: Record<string, { title: string; subtitle: string }> = {
  ...
  perfil:       { title: 'Mi Perfil',        subtitle: 'Información personal y firma'   },
};
...
export function Topbar({ onToggleSidebar }: TopbarProps) {
  ...
  return (
    <header style={{
      ...
      <div style={{ flex: 1 }}>
        <h1 style={{ margin: 0, fontSize: 15, fontWeight: 600, color: '#0f172a', lineHeight: 1.2 }}>{page.title}</h1>
        <p style={{ margin: 0, fontSize: 11, color: '#94a3b8', lineHeight: 1.2 }}>{page.subtitle}</p>
      </div>
      ...
      <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 34, height: 34, borderRadius: 8, border: '1px solid #e2e8f0', background: 'transparent', cursor: 'pointer', color: '#64748b' }}>
        <Bell size={16} />
      </button>
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setMenuOpen(o => !o)} style={{
          ...
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg, #1F4788, #3b82f6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
            {inicial}
          </div>
          <div style={{ textAlign: 'left', lineHeight: 1.25 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: '#0f172a' }}>{usuario?.nombre_completo ?? 'Usuario'}</p>
            <p style={{ margin: 0, fontSize: 10, color: '#94a3b8', textTransform: 'capitalize' }}>{usuario?.rol?.nombre ?? ''}</p>
          </div>
        ...
        {menuOpen && (
          <>
            ...
            <div style={{ position: 'absolute', right: 0, top: 'calc(100% + 8px)', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 12, boxShadow: '0 10px 40px rgba(0,0,0,0.12)', minWidth: 190, zIndex: 100, overflow: 'hidden' }}>
              <div style={{ padding: '12px 14px', borderBottom: '1px solid #f1f5f9' }}>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: '#0f172a' }}>{usuario?.nombre_completo}</p>
                <p style={{ margin: '2px 0 0', fontSize: 11, color: '#94a3b8' }}>{usuario?.email ?? usuario?.username}</p>
              </div>
              <div style={{ padding: 6 }}>
                <DropItem icon={<User size={14} />} label="Mi perfil"
...
function DropItem({ icon, label, onClick, danger = false }: { icon: React.ReactNode; label: string; onClick: () => void; danger?: boolean; }) {
  return (
    <button onClick={onClick} style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 10px', borderRadius: 8, border: 'none', background: 'transparent', cursor: 'pointer', textAlign: 'left', fontSize: 13, color: danger ? '#ef4444' : '#475569' }}
