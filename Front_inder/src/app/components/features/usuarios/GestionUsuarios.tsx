...
import { Plus, Edit2, Trash2, UserCheck, UserX, Loader2, X, Shield, Users, ChevronDown, ChevronUp, Save } from 'lucide-react';
...
export function GestionUsuarios() {
  ...
  // Edición de permisos de rol existente
  ...
  const toggleActivo = async (u: Usuario) => {
    try {
      ...
      toast.success(u.activo ? 'Usuario desactivado' : 'Usuario activado'); cargar();
  ...
  const abrirCrearRol = () => { setFormRol({ nombre: '', descripcion: '' }); setPermisosRol({}); setModalRol(true); };

  const togglePermiso = (modulo: string, accion: string) => {
    setPermisosRol(prev => {
      const acciones = prev[modulo] ?? [];
      return { ...prev, [modulo]: acciones.includes(accion) ? acciones.filter(a => a !== accion) : [...acciones, accion] };
    });
  };

  const toggleModuloCompleto = (modulo: string) => {
    const acciones = permisosRol[modulo] ?? [];
    setPermisosRol(prev => ({ ...prev, [modulo]: acciones.length === ACCIONES.length ? [] : [...ACCIONES] }));
  };

  const guardarRol = async () => {
    if (!formRol.nombre) { toast.error('El nombre es requerido'); return; }
    const permisos = Object.entries(permisosRol).flatMap(([modulo, acciones]) => acciones.map(accion => ({ modulo, accion })));
    try {
      setSaving(true);
      await api.post('/roles', { nombre: formRol.nombre, descripcion: formRol.descripcion, permisos });
      toast.success('Rol creado'); setModalRol(false); cargar();
    } catch (err: any) { toast.error(err?.response?.data?.detail ?? 'Error al crear rol'); }
    finally { setSaving(false); }
  };

  // ── Editar permisos de rol existente ─────────────────────────
  const abrirEditarPermisos = (rol: Rol) => {
    const mapa: Record<string, string[]> = {};
    MODULOS.forEach(m => { mapa[m] = rol.permisos.filter(p => p.modulo === m).map(p => p.accion); });
    setPermisosEdit(mapa);
    setEditandoPermisos(rol.id);
    setRolExpandido(rol.id); // expandir al editar
  };

  const togglePermisoEdit = (modulo: string, accion: string) => {
  ...
  const toggleModuloEdit = (modulo: string) => {
    const acciones = permisosEdit[modulo] ?? [];
    setPermisosEdit(prev => ({ ...prev, [modulo]: acciones.length === ACCIONES.length ? [] : [...ACCIONES] }));
  };

  const guardarPermisos = async (rolId: string) => {
    const permisos = Object.entries(permisosEdit).flatMap(([m, as]) => as.map(a => ({ modulo: m, accion: a })));
    try {
      setSavingPermisos(true);
      await api.put(`/roles/${rolId}/permisos`, { permisos });
      toast.success('Permisos actualizados'); setEditandoPermisos(null); cargar();
    } catch (err: any) { toast.error(err?.response?.data?.detail ?? 'Error al guardar permisos'); }
  ...
  const tabStyle = (active: boolean) => ({
    ...
    background: active ? '#1F4788' : 'transparent', color: active ? '#fff' : '#64748b',
  ...
  // Matriz de permisos reutilizable
  const MatrizPermisos = ({ perms, onToggle, onToggleAll }: {
    perms: Record<string, string[]>;
    onToggle: (m: string, a: string) => void;
    onToggleAll: (m: string) => void;
  }) => (
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ background: '#f8fafc' }}>
            ...
            {ACCIONES.map(a => <th key={a} style={{ padding: '9px 10px', textAlign: 'center', fontSize: 12, fontWeight: 600, color: '#475569', borderBottom: '1px solid #e2e8f0', textTransform: 'capitalize' }}>{a}</th>)}
              ...
              <tr key={modulo} style={{ background: idx % 2 === 0 ? '#fff' : '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                ...
                {ACCIONES.map(accion => (
                  <td key={accion} style={{ padding: '10px', textAlign: 'center' }}>
                    <input type="checkbox" checked={sel.includes(accion)} onChange={() => onToggle(modulo, accion)}
                      style={{ width: 15, height: 15, cursor: 'pointer', accentColor: '#1F4788' }} />
                  </td>
                ))}
                <td style={{ padding: '10px', textAlign: 'center' }}>
                  <input type="checkbox" checked={sel.length === ACCIONES.length} onChange={() => onToggleAll(modulo)}
                    style={{ width: 15, height: 15, cursor: 'pointer', accentColor: '#1F4788' }} />
                </td>
              </tr>
            );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 13, color: '#64748b' }}>
            {tab === 'usuarios' ? 'Gestiona los usuarios del sistema' : 'Define roles y edita sus permisos por módulo'}
          </p>
        </div>
        <button onClick={tab === 'usuarios' ? abrirCrearUsuario : abrirCrearRol}
      ...
      <div style={{ display: 'flex', gap: 4, background: '#f8fafc', padding: 4, borderRadius: 10, width: 'fit-content', border: '1px solid #e2e8f0' }}>
        <button style={tabStyle(tab === 'usuarios')} onClick={() => setTab('usuarios')}><Users size={14} /> Usuarios ({usuarios.length})</button>
        <button style={tabStyle(tab === 'roles')} onClick={() => setTab('roles')}><Shield size={14} /> Roles ({roles.length})</button>
      </div>
            ...
            <tbody>
              {usuarios.map(u => (
                <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <span style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, background: `${getRolColor(u.rol.nombre)}15`, color: getRolColor(u.rol.nombre), fontWeight: 600 }}>{u.rol.nombre}</span>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
          ...
          {roles.map(rol => (
            <div key={rol.id} style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px' }}>
                <button onClick={() => setRolExpandido(r => r === rol.id ? null : rol.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 14, flex: 1, background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <p style={{ margin: '2px 0 0', fontSize: 12, color: '#64748b' }}>{rol.descripcion ?? `Rol ${rol.nombre}`} · {rol.permisos.length} permisos</p>
                  </div>
                ...
                <button
                  onClick={() => editandoPermisos === rol.id ? setEditandoPermisos(null) : abrirEditarPermisos(rol)}
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 12px', background: editandoPermisos === rol.id ? '#EEF3FB' : '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 600, color: '#1F4788', flexShrink: 0 }}>
                  <Edit2 size={12} /> {editandoPermisos === rol.id ? 'Cancelar' : 'Editar permisos'}
                </button>
              </div>

              {/* Editor de permisos */}
              {editandoPermisos === rol.id && (
                <div style={{ borderTop: '1px solid #f1f5f9', padding: '16px 18px', background: '#fafbff' }}>
                  <p style={{ margin: '0 0 12px', fontSize: 12, fontWeight: 600, color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Editando permisos — <span style={{ color: getRolColor(rol.nombre), textTransform: 'capitalize' }}>{rol.nombre}</span>
                  </p>
                  <MatrizPermisos perms={permisosEdit} onToggle={togglePermisoEdit} onToggleAll={toggleModuloEdit} />
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 14, gap: 8 }}>
                    <button onClick={() => setEditandoPermisos(null)}
                      style={{ padding: '8px 16px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 8, cursor: 'pointer', fontSize: 13 }}>
                      Cancelar
                    </button>
                    <button onClick={() => guardarPermisos(rol.id)} disabled={savingPermisos}
                      style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '9px 20px', background: '#1F4788', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                      {savingPermisos ? <Loader2 size={14} style={{ animation: 'spin 0.8s linear infinite' }} /> : <Save size={14} />}
              ...
              {/* Vista de permisos actuales (expandido, no editando) */}
              {rolExpandido === rol.id && editandoPermisos !== rol.id && (
                      ...
                      return (
                        <div key={modulo} style={{ padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}>
                          ...
                          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                            {accionesRol.map(a => <span key={a} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 10, background: '#EEF3FB', color: '#1F4788', fontWeight: 600 }}>{a}</span>)}
                          </div>
                        </div>
                      );
                    })}
      ...
      {/* ── MODAL USUARIO ── */}
      {modalUsuario && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }} onClick={() => setModalUsuario(false)}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 440, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                ...
                <div key={key}>
                  ...
                  <input type={type} value={(formUsuario as any)[key]} onChange={e => setFormUsuario(f => ({ ...f, [key]: e.target.value }))} disabled={disabled}
      ...
      {/* ── MODAL ROL ── */}
      {modalRol && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 16 }} onClick={() => setModalRol(false)}>
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  ...
                  <input value={formRol.nombre} onChange={e => setFormRol(f => ({ ...f, nombre: e.target.value }))} placeholder="ej: fisioterapeuta"
                ...
                <div>
                  ...
                  <input value={formRol.descripcion} onChange={e => setFormRol(f => ({ ...f, descripcion: e.target.value }))} placeholder="Descripción opcional"
                    style={{ width: '100%', padding: '9px 11px', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: 13, boxSizing: 'border-box', outline: 'none' }} />
                </div>
              </div>
              <div>
                ...
                <MatrizPermisos perms={permisosRol} onToggle={togglePermiso} onToggleAll={toggleModuloCompleto} />
              </div>
            </div>
