...
const inputStyle = {
  ...
  boxSizing:'border-box' as const, background:T.surface,
};
...
// ── Componentes atómicos FUERA del componente principal ───────
// Definirlos dentro causaría re-montaje en cada keystroke.

function Seccion({ titulo, icon, sec, children }: {
...
function ItemAntecedente({ codigo, nombre, extra, color, onDelete }: {
...
function Sugerencias({ items, color, onSelect }: {
  items: {codigo:string;nombre:string}[]; color: string;
...
function BtnAgregar({ label, color, onClick }: { label: string; color: string; onClick: () => void }) {
  return (
...
function RadioSiNo({ name, value, onSi, onNo }: { name: string; value: boolean | null; onSi: () => void; onNo: () => void }) {
  return (
...
function FormCIE({ codigo, nombre, err, sugCod, showSugCod, sugNom, showSugNom, color,
...
export function AntecedentesMedicos({ data, updateData, onNext, onPrevious, onCancel }: Props) {
  ...
  const [sugNomP, setSugNomP] = useState<{codigo:string;nombre:string}[]>([]);
  const [showSugNomP, setShowSugNomP] = useState(false);
  const [sugCodP, setSugCodP] = useState<{codigo:string;nombre:string}[]>([]);
  const [showSugCodP, setShowSugCodP] = useState(false);

  // Familiar
  const [codigoF, setCodigoF] = useState("");
  const [nombreF, setNombreF] = useState("");
  const [familiar, setFamiliar] = useState("");
  const [obsF,    setObsF]    = useState("");
  const [errF,    setErrF]    = useState("");
  const [sugNomF, setSugNomF] = useState<{codigo:string;nombre:string}[]>([]);
  const [showSugNomF, setShowSugNomF] = useState(false);
  const [sugCodF, setSugCodF] = useState<{codigo:string;nombre:string}[]>([]);
  const [showSugCodF, setShowSugCodF] = useState(false);

  // Debounce refs — usar useRef para no recrearlos en cada render
  const debP = { current: null as any };
  const debF = { current: null as any };

  const buscarCodigo = async (
    v: string,
    setCod: (s:string) => void, setNom: (s:string) => void,
    setErr: (s:string) => void,
    setSugCod: (s:{codigo:string;nombre:string}[]) => void,
    setShowSugCod: (b:boolean) => void,
  ) => {
    ...
    if (u.trim()) {
      ...
      if (e) {
        setNom(e?.nombre ?? "");
        setSugCod([]); setShowSugCod(false);
      } else {
        const r = await buscarPorCodigoParcial(u);
        if (r.length > 0) {
          setSugCod(r.map((x:any) => ({ codigo:x.codigo, nombre:x.nombre })));
          setShowSugCod(true);
        } else {
      ...
      setNom(""); setSugCod([]); setShowSugCod(false);
    }
  };
  ...
  const buscarNombre = (
    ...
    setErr: (s:string) => void,
    setSugNom: (s:{codigo:string;nombre:string}[]) => void,
    setShowSugNom: (b:boolean) => void,
  ) => {
    setNom(v); setErr("");
    ...
    if (v.trim().length >= 3) {
      deb.current = setTimeout(async () => {
        ...
        if (r.length > 0) {
          setSugNom(r.map((x:any) => ({ codigo:x.codigo, nombre:x.nombre })));
          setShowSugNom(true);
        } else {
          setSugNom([]); setShowSugNom(false);
        }
      }, 400);
    } else {
      setSugNom([]); setShowSugNom(false);
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
      ...
      <Seccion titulo="Antecedentes Personales" icon={<User size={15}/>} sec={SEC.personal}>
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <FormCIE
            codigo={codigoP} nombre={nombreP} err={errP}
            sugCod={sugCodP} showSugCod={showSugCodP}
            sugNom={sugNomP} showSugNom={showSugNomP}
            color={SEC.personal.icon}
      ...
      <Seccion titulo="Antecedentes Familiares" icon={<Users size={15}/>} sec={SEC.familiar}>
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <FormCIE
            codigo={codigoF} nombre={nombreF} err={errF}
            sugCod={sugCodF} showSugCod={showSugCodF}
            sugNom={sugNomF} showSugNom={showSugNomF}
            color={SEC.familiar.icon}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <Campo label="Observaciones">
              <input value={obsF} onChange={e => setObsF(e.target.value)}
                placeholder="Detalles adicionales..." style={inputStyle}/>
            </Campo>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:12, padding:14, background:T.surface, borderRadius:T.radiusSm, border:`1px solid ${T.border}` }}>
            <Campo label="Descripción de lesiones">
              <textarea value={data.descripcionLesiones} rows={3}
                ...
                placeholder="Describa las lesiones (tipo, gravedad, zona afectada)..."
                style={textareaStyle}/>
            </Campo>
      ...
      <Seccion titulo="Cirugías previas" icon={<Scissors size={15}/>} sec={SEC.cirugias}>
        ...
        {data.cirugiasPrevias && (
          <textarea value={data.detalleCirugias} rows={3}
            ...
            placeholder="Detalle las cirugías (tipo, fecha, resultados)..."
            style={textareaStyle}/>
        )}
      </Seccion>
      ...
      <Seccion titulo="Medicación actual" icon={<Pill size={15}/>} sec={SEC.medicacion}>
        ...
        {data.tomaMedicacion && (
          <input value={data.medicacionActual}
      ...
      <Seccion titulo="Vacunas" icon={<Syringe size={15}/>} sec={SEC.vacunas}>
        <p style={{ margin:'0 0 12px', fontSize:12, color:T.textSecondary }}>
          Vacunas registradas del deportista con sus certificados
        </p>
        {data.deportista_id && typeof data.deportista_id === 'string' ? (
          ...
          <p style={{ textAlign:'center', fontSize:12, color:T.textMuted, fontStyle:'italic' }}>
