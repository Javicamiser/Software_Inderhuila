...
import { Plus, Edit2, Trash2, Eye, Search, X, ArrowLeft } from 'lucide-react';
...
const getDisciplinaLogo = (disciplina: string | undefined) => {
  ...
  if (disciplinaLower.includes('subacuatica') || disciplinaLower.includes('buceo')) {
    console.log('Coincide: Subacuatica');
    return { 
...
export function ListadoDeportistas({ onNavigate }: ListadoDeportistasProps) {
  ...
  if (mostrarRegistro) {
    return (
      <div>
        <button
          onClick={() => { setMostrarRegistro(false); cargarDeportistas(); }}
        ...
        <RegistroDeportista onCancel={() => { setMostrarRegistro(false); cargarDeportistas(); }} />
      </div>
    );
  }
