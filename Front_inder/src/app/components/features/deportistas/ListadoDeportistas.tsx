...
const LOGOS: Record<string, string> = {
  natacion: logoNatacion, natación: logoNatacion,
  pesas: logoPesas,
  lucha: logoLucha,
  subacuatico: logoSubacuatico, subacuático: logoSubacuatico,
};

const getLogo = (deporte?: string) => {
  if (!deporte) return null;
  const key = deporte.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  return Object.entries(LOGOS).find(([k]) => key.includes(k))?.[1] ?? null;
};
...
export function ListadoDeportistas({ onNavigate }: ListadoDeportistasProps) {
  ...
  const eliminar = async (dep: Deportista) => {
    if (!confirm(`¿Eliminar a ${dep.nombres} ${dep.apellidos}?`)) return;
    try {
      await deportistasService.remove(dep.id);
...
<input type="text" value={form.tipo_deporte || ''} onChange={e => setForm((f:any) => ({ ...f, tipo_deporte: e.target.value }))}
                placeholder="ej: Natación, Pesas, Lucha..."
                style={{ width:'100%', padding:'9px 11px', border:`1px solid ${T.border}`, borderRadius:T.radiusSm, fontSize:13, boxSizing:'border-box', outline:'none' }}
                onFocus={e => e.currentTarget.style.borderColor = T.primary}
                onBlur={e => e.currentTarget.style.borderColor = T.border}
              />
          ...
          <table style={{ width:'100%', borderCollapse:'collapse' }}>
            ...
            <tbody>
              {filtrados.map((dep, i) => {
                ...
                return (
                  <tr key={dep.id} style={{ borderBottom:`1px solid ${T.borderLight}`, background: i%2===0 ? T.surface : T.surfaceAlt, transition:'background 0.1s' }}
                  ...
                  >
                    ...
                    <td style={{ padding:'13px 16px' }}>
                      <div style={{ display:'flex', gap:4, justifyContent:'center' }}>
                        ...
                        <button onClick={() => eliminar(dep)} title="Eliminar"
